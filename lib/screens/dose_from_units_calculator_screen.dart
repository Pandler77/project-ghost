...
class DoseFromUnitsCalculatorScreen extends StatefulWidget {
  ...
  @override
  State<DoseFromUnitsCalculatorScreen> createState() =>
      _DoseFromUnitsCalculatorScreenState();
}

class _DoseFromUnitsCalculatorScreenState
    extends State<DoseFromUnitsCalculatorScreen> {
  ...
  final TextEditingController _syringeUnitsController = TextEditingController();

  String _vialUnit = 'mg';
  double _selectedSyringeMl = 1.0;

  double? _resultDoseMcg;
  double? _resultVolumeMl;
  ...
  void dispose() {
    ...
    _syringeUnitsController.dispose();
  ...
  void _calculate() {
    ...
    final syringeUnits = double.tryParse(_syringeUnitsController.text.trim());
    ...
    if (syringeUnits == null || syringeUnits <= 0) {
      _showError('Enter valid syringe units.');
      return;
    }

    final maximumUnits = _selectedSyringeMl * 100;

    if (syringeUnits > maximumUnits) {
      _showError(
        '${_formatNumber(syringeUnits)} units exceeds the selected '
        '${_formatNumber(_selectedSyringeMl)} mL syringe.',
      );
      return;
    }
    ...
    // 100 units = 1 mL.
    final volumeMl = syringeUnits / 100;

    if (volumeMl > liquidVolume) {
      _showError(
        'The selected volume is larger than the total liquid in the vial.',
      );
      return;
    }

    final concentrationPerMl = vialAmountInMcg / liquidVolume;
    final doseMcg = concentrationPerMl * volumeMl;
    final dosesPerVial = vialAmountInMcg / doseMcg;

    setState(() {
      _resultDoseMcg = doseMcg;
      _resultVolumeMl = volumeMl;
  ...
  String get _primaryDoseDisplay {
    final doseMcg = _resultDoseMcg;
    ...
    if (doseMcg >= 1000) {
      return '${_formatNumber(doseMcg / 1000)} mg';
    }

    return '${_formatNumber(doseMcg)} mcg';
  }

  String get _secondaryDoseDisplay {
    final doseMcg = _resultDoseMcg;
    ...
    return '${_formatNumber(doseMcg / 1000, decimalPlaces: 6)} mg';
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Dose from Units'),
        actions: [
          IconButton(
            onPressed: _clear,
      ...
      body: SafeArea(
        child: ListView(
          ...
          children: [
            const Text(
              'Calculate dose from syringe units',
              style: TextStyle(
            ...
            Text(
              'Enter the vial amount, liquid added, and units drawn.',
              style: TextStyle(
            ...
            TextField(
              controller: _syringeUnitsController,
              keyboardType: const TextInputType.numberWithOptions(
                decimal: true,
              ),
              decoration: const InputDecoration(
                labelText: 'Syringe units drawn',
                hintText: '45',
                suffixText: 'units',
                border: OutlineInputBorder(),
              ),
                ...
                child: Column(
                  children: [
                    ...
                    SyringeScaleVisualization(
                      units: double.parse(_syringeUnitsController.text.trim()),
                ...
                child: Column(
                  children: [
                    _ResultRow(
                      ...
                      label: 'Volume drawn',
                      value:
                          '${_formatNumber(_resultVolumeMl!, decimalPlaces: 4)} mL',
                    ),
      ...
      children: [
        Text(
          exceedsCapacity
              ...
              : 'Filled to the ${_formatUnits(units)} unit line',
          textAlign: TextAlign.center,
          style: TextStyle(
            fontSize: AppTypography.body,
            fontWeight: FontWeight.w700,
