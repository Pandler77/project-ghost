            ...
            children: [
              Expanded(
                child: _HeroMetric(
                  label: 'TOTAL SUPPLY',
                  value:
                      '${_formatAmount(totalInventoryAmount)} ${item.unit}',
                ),
              ),
      ...
      child: Column(
        ...
        children: [
          ...
          if (available.isNotEmpty) ...[
            const SizedBox(height: AppSpacing.md),
            for (var index = 0; index < available.length; index++) ...[
              _BatchInventoryTile(batch: available[index], accent: accent),
              if (index < available.length - 1)
                const SizedBox(height: AppSpacing.sm),
            ],
          ],
...
class _BatchInventoryTile extends StatelessWidget {
  const _BatchInventoryTile({required this.batch, required this.accent});
        ...
        children: [
          ...
          Text(
            hasForecast
                ...
                : 'Ghost needs compatible dose and inventory units.',
...
IconData _eventIcon(InventoryEventType type) {
  return switch (type) {
    InventoryEventType.created => Icons.add_box_outlined,
    InventoryEventType.doseDeducted => Icons.remove_circle_outline,
    InventoryEventType.doseRestored => Icons.restore,
    InventoryEventType.vialOpened => Icons.science_outlined,
    InventoryEventType.stockAdded => Icons.inventory_2_outlined,
    InventoryEventType.manualAdjustment => Icons.tune,
    InventoryEventType.archived => Icons.archive_outlined,
  };
}
