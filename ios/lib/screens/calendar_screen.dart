...
class _CalendarScreenState extends State<CalendarScreen> {
  ...
  CalendarDaySummary _summaryForDate({
    ...
    final scheduledProtocols = _protocolsForDate(date);
    ...
    // Historical/manual doses must appear even when that protocol was not
    // scheduled on the selected date.
    final dateRecords = records
        .where((record) => isSameDay(record.scheduledFor, date))
        .toList();

    final protocolMap = <String, Protocol>{
      for (final protocol in scheduledProtocols) protocol.id: protocol,
    };
    ...
    return CalendarDaySummary(
      date: date,
      protocols: protocolMap.values.toList(),
      records: dateRecords,
