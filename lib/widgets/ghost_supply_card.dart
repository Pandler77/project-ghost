...
class GhostSupplyCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final hasInventory = items.isNotEmpty;
    ...
    final content = Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          if (!hasInventory)
            _EmptySupplyState(onTap: onTap)
          else ...[
            for (var index = 0; index < visibleItems.length; index++) ...[
              _SupplyRow(
                item: visibleItems[index],
                protocolName: _protocolName(visibleItems[index].protocolId),
                batches: batchesByItemId[visibleItems[index].id] ?? const [],
              ),
    ...
    return Card(
      ...
      child: hasInventory ? InkWell(onTap: onTap, child: content) : content,
    );
  }
...
class _EmptySupplyState extends StatelessWidget {
  const _EmptySupplyState({required this.onTap});
